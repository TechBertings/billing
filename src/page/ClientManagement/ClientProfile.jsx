...
const formatTIN = (val) => {
  ...
  if (f.length > 9)
    f = `${f.slice(0, 3)}-${f.slice(3, 6)}-${f.slice(6, 9)}-${f.slice(9)}`;
        ...
        return (
          <React.Fragment key={step.id}>
            ...
            >
              ...
              <span
                className={`text-xs font-medium transition-all hidden sm:block whitespace-nowrap
      ...
      const { error } = await supabase.from("client_profile").insert([
        {
          ...
          sole_proprietor_signatory_type:
            formData.sole_proprietor_signatory_type,
          ...
          official_office_contact_number:
            formData.official_office_contact_number,
      ...
      case 4:
        return (
          ...
          ${formData.renting === opt ? "border-blue-500 bg-blue-500" : "border-slate-300"}`}
                    >
                      {formData.renting === opt && (
                        <div className="w-1.0 h-1.0 rounded-full bg-white" />
                      )}
                    </div>
              ...
              <div className="overflow-hidden border-2 border-slate-200 rounded-xl">
                ...
                <div className="flex items-center justify-between px-4 py-2 border-t bg-slate-50 border-slate-200">
                  <p className="text-xs text-slate-400">
                    💡 Type an address above then press <strong>Enter</strong>{" "}
                    or click <strong>Search</strong>
                  </p>
                  {formData.google_map_location && (
              ...
              <div className="flex gap-4 mt-1">
                {["Corporation", "Sole Proprietor"].map((type) => (
                  <label
                    key={type}
                    onClick={() =>
                      setFormData((p) => ({ ...p, entity_type: type }))
                    }
